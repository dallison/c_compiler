...
static bool ExecuteInstruction(X86_64Interpreter* interpreter, size_t* insn_len,
  ...
  *rip_updated = false;
  ...
  // Consume legacy / mandatory SSE prefixes and LOCK. Guest scheduling does
  // not preempt an instruction, so LOCK needs no additional synchronization.
  ...
  if (b0 == 0x0F) {
    ...
    if (b1 == 0xC0 || b1 == 0xC1) {  // XADD r/m, r
      ModRM modrm;
      if (!DecodeModRM(interpreter, &pos, rex, true, &modrm)) {
        return false;
      }
      int width = b1 == 0xC0 ? 1 : (rex.w ? 8 : (sse_prefix == 0x66 ? 2 : 4));
      ...
      uint64_t old;
      if (modrm.mod == 3) {
        old = ReadReg(interpreter, modrm.rm) & mask;
        uint64_t destination = (old + source) & mask;
        uint64_t current = ReadReg(interpreter, modrm.rm);
        WriteReg(interpreter, modrm.rm,
                 width == 4 ? destination
