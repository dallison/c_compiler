...
static void BreakpointInit(Breakpoint* bp, int num, uint64_t address,
                           bool is_temp) {
...
static Breakpoint* FindBreakpointAtAddress(ARMDebugger* debugger,
                                           uint64_t addr) {
  for (ListElement* element = debugger->breakpoints.first; element != NULL;
...
static Breakpoint* FindBreakpointAtCurrentPC(ARMDebugger* debugger) {
  return FindBreakpointAtAddress(debugger, debugger->interpreter->pc);
}

static Breakpoint* FindBreakpointByNumber(ARMDebugger* debugger, int bpnum) {
  for (ListElement* element = debugger->breakpoints.first; element != NULL;
       element = element->next) {
    Breakpoint* bp = (Breakpoint*)element;
    if (bp->num == bpnum) {
...
static void RestoreBreakpointInstruction(Breakpoint* bp) {
  *(int32_t*)(uintptr_t)bp->address = bp->instruction;
}

static void InsertBreakpoint(Breakpoint* bp) {
  bp->instruction = *(int32_t*)(uintptr_t)bp->address;
  *(int32_t*)(uintptr_t)bp->address = (int32_t)ARM_BREAKPOINT_INSN;
}

static void CreateBreakpoint(ARMDebugger* debugger, int64_t addr, bool is_temp) {
...
static void ShowLocation(ARMDebugger* debugger, uint64_t pc, SymbolScope* symbol,
                         int frame_id) {
  printf("%-3s", frame_id == debugger->current_frame_id ? "=>" : "");
  printf("%-3d 0x%08" PRIx64 " %s\n", frame_id, pc,
         symbol == NULL ? "" : symbol->name);
}

static void BreakpointHit(ARMDebugger* debugger) {
...
void ARMDebuggerCycle(ARMDebugger* debugger) {
  for (;;) {
    if (setjmp(debugger->interpreter->debugger) == 0) {
      ARMInterpreterCycle(debugger->interpreter);
      return;
    } else {
...
static uint64_t FindSymbolAddress(ARMDebugger* debugger, const char* name) {
...
static struct {
  ...
  const char* help;
    ...
    {"trace", kCommandTrace, "Switch instruction tracing on or off"},
...
static Command GetCommand(ARMDebugger* debugger, char* buf, int buflen,
                          char** end) {
  printf("arm> ");
  fflush(stdout);
...
static void Run(ARMDebugger* debugger, char* tail) {
  (void)tail;
  if (setjmp(debugger->main_loop) == 0) {
    debugger->interpreter->num_steps = -1;
    ARMDebuggerCycle(debugger);
  }
}

static void Continue(ARMDebugger* debugger, char* tail) {
  (void)tail;
  if (setjmp(debugger->main_loop) == 0) {
    Breakpoint* bp = FindBreakpointAtCurrentPC(debugger);
    if (bp != NULL) {
...
static void Step(ARMDebugger* debugger, char* tail) {
  (void)tail;
  if (setjmp(debugger->main_loop) == 0) {
    debugger->interpreter->num_steps = 1;
    Breakpoint* bp = FindBreakpointAtCurrentPC(debugger);
...
static void Next(ARMDebugger* debugger, char* tail) {
  Step(debugger, tail);
}

static void SetBreakpoint(ARMDebugger* debugger, char* tail) {
...
static void DelBreakpoint(ARMDebugger* debugger, char* tail) {
  char* end;
...
static void Print(ARMDebugger* debugger, char* tail) {
  (void)debugger;
  (void)tail;
}

static void Dump(ARMDebugger* debugger, char* tail) {
  (void)debugger;
  int size = 10;
  char* p = tail;
  if (*tail == '/') {
  ...
  char buf[16];
  const char* caddr = (const char*)(uintptr_t)addr;
...
static void ShowRegs(ARMDebugger* debugger, char* tail) {
  (void)tail;
  ARMInterpreterDumpRegisters(debugger->interpreter);
}

static void Disassemble(ARMDebugger* debugger, char* tail) {
  char* p;
  ...
  while (addr < end) {
    ...
    printf("0x%08" PRIx64 ": %08x\n", addr, insn);
    addr += 4;
  }
}

static void Backtrace(ARMDebugger* debugger, char* tail) {
  (void)tail;
  SymbolScope curr;
...
static void Trace(ARMDebugger* debugger, char* tail) {
  if (strcmp(tail, "on") == 0) {
...
static void Help(ARMDebugger* debugger, char* tail) {
  (void)debugger;
  (void)tail;
  for (size_t i = 0; commands[i].spelling != NULL; i++) {
...
static void RecordCommand(ARMDebugger* debugger, const char* command) {
  StringSet(&debugger->last_command, command);
}

static void ClearCommand(ARMDebugger* debugger) {
