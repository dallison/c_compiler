...
int main(void) {
  ...
  ChronoSysInfoRequest sys_request = {
      (uint64_t)(uintptr_t)&sys_seconds, (uint64_t)(uintptr_t)&sys_info,
      (uint64_t)(uintptr_t)abbrevs, CHRONO_ABBREV_MAX, 0};
  ChronoLocalInfoRequest local_request = {
      (uint64_t)(uintptr_t)&local_seconds, (uint64_t)(uintptr_t)&local_info,
      (uint64_t)(uintptr_t)abbrevs, CHRONO_ABBREV_MAX, 0};
  ...
  sys_seconds = 100;
  if (syscall(SYS_TZDB_SYS_INFO, "FixedOffset", &sys_request) != 0) return 7;
  if (sys_info.offset_seconds != 3600) return 8;
  if (strcmp(abbrevs, "FIX") != 0) return 9;
  sys_seconds = 1712000000;
  if (syscall(SYS_TZDB_SYS_INFO, "DST", &sys_request) != 0) return 10;
  if (sys_info.offset_seconds != 3600) return 11;
  if (syscall(SYS_TZDB_LOCAL_INFO, "DST", &local_request) != 0) return 12;
