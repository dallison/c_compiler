...
int main() {
  ...
  const sys_time round_trip = utc_clock::to_sys(utc_clock::from_sys(sys_before));
  if (round_trip.time_since_epoch() != sys_before.time_since_epoch()) {
    return 4;
  }

  const sys_time from_file = file_clock::to_sys(file_before);
  ...
  const utc_time utc_from_tai = tai_clock::to_utc(tai_before);
  ...
  const utc_time utc_from_gps = gps_clock::to_utc(gps_before);
