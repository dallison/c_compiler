...
//  c_compiler
//
//  Created by David Allison on 12/15/21.
//  Copyright © 2021 David Allison. All rights reserved.
//

// This is the ROM for the 6502 interpreter.  It is loaded at 0xc000
// and provides hosted operating system support functions.
.text
...
// File descriptor table is the last 40 bytes of page 3.
.set fd_table 0x400 - fd_size*max_fds

// Errno is just below the fd table.
.set errno 0x3d6
...
.set device_lseek_func 12

.set acia_csr 0xfe00
.set acia_data 0xfe01

.set stdin_device_id 0
.set stdout_device_id 1
.set file_device_id 2
...
#if 0       // In the interpreter, TDRE is always true.
acia_wc_loop:
  LDA acia_csr
  AND #2             // Check TDRE
  BEQ acia_wc_loop
...
close:
  JSR check_fd_open
  LDA #device_close_func
  JSR get_device_func
  JMP func_call

lseek:
  JSR check_fd_open
  LDA #device_lseek_func
  JSR get_device_func
  JMP func_call

ioctl:
  JSR check_fd_open
  LDA #device_ioctl_func
  JSR get_device_func
  JMP func_call

poll:
  JSR check_fd_open
  LDA #device_poll_func
  JSR get_device_func
  JMP func_call
...
.set syscall_code 0xfc // 2 bytes
.set syscall_vector 0xfe
...
// On reset, the interpreter has put the start address of the program
// into 0,1.  We just jump there after initializing the file descriptor
// table.
.global _start
_start:
reset_handler:
