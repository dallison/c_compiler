...
static void TemplateArgumentVectorDelete(Vector* args) {
  if (args == NULL) {
    return;
  }
...
static Vector* TemplateArgumentVectorListCopy(Vector* list) {
  if (list == NULL) {
    return NULL;
  }
  Vector* copy = NewVector();
...
static void CloneInstantiatedMemberFunctionBody(TypeParser* parser,
                                                ...
                                                Struct* substitution_source,
                                                Vector* args);
static CXXConstructorInitList* FindTemplateConstructorInitializers(
    Symbol* symbol);
...
static Vector* SplitDependentMemberPath(String* path) {
...
static void DeleteStringVector(Vector* strings) {
  if (strings == NULL) {
    return;
  }
...
static void FinalizePendingInlineConstructorPreambles(TypeParser* parser,
                                                     Struct* owner) {
...
static void AppendCXXMemberwiseAssignments(TypeParser* parser,
                                           TypeRecord* func, Vector* body,
                                           Struct* owner,
                                           SourceLocation location);
