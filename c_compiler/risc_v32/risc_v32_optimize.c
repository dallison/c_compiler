...
// This file contains functions to optimize the instruction sequence for RISC-V32.
// Now that we have lowered the IR to actual RISC-V32 instructions we can look for
...
static bool RV32HasImplicitEffect(TargetInstruction* inst) {
...
static void RemoveBlockUnusedExpressions(TargetBasicBlock* block, void* data) {
  ...
  for (TargetInstruction* inst = TargetBasicBlockRBegin(block);
    ...
    if (!inst->observable_checkpoint && !RV32HasImplicitEffect(inst) &&
        RV32IsExpression(inst) &&
        !RV32IsSymbol(inst) && !RV32IsConst(inst) && opcode != RV32_OP(tmp) &&
        opcode != RV32_OP(sp)) {
      ...
      if (dest == NULL && inst->operand[1] != NULL &&
          (opcode == RV32_OP(mv) || opcode == RV32_OP(fmv_s) ||
      ...
      if (dest != NULL && (((int)dest->opcode == (int)RV32_OP(tmp)) ||
          ((int)dest->opcode == (int)RV32_OP(sp)) ||
          RV32IsFixedRegister(dest) ||
      ...
      if (is_candidate && ((int)inst->opcode != (int)RV32_OP(tmp)) &&
          !RV32IsFixedRegister(inst) &&
...
#if 0
    } else if (((int)inst->opcode == (int)RV32_OP(rmov))) {
       ...
       if (RV32IsResult(result)) {
         goto dont_optimize;
       }
       if ((RV32Opcode)((int)result->opcode == (int)RV32_OP(sp))) {
         goto dont_optimize;
       }
      ...
      if (RV32IsArgRegister(src) &&
          !BitSetContains(&filter, src->id) &&
          !BitSetContains(&block->output_ids, result->id)) {
#if 0
        if (RV32IsFixedRegister((RV32Opcode)result->opcode)) {
...
static void RemoveUnusedExpressions(RV32Generator* rv) {
...
static void CombineLoadOrStoresInBlock(TargetBasicBlock* block, void* data) {
  ...
  for (TargetInstruction* inst = TargetBasicBlockRBegin(block);
    ...
    if (RV32IsLoad(inst)) {
      ...
      if (base->opcode == (TargetOpcode)RV32_OP(la) &&
          !compiler->pic && RV32IntValue(inst->operand[1]) == 0 &&
          base->users.length == 1) {
        ...
        TargetInstruction* label =
            TargetNewInstruction((TargetOpcode)RV32_OP(label));
        label->flags = RV32_EXPORTED_LABEL;
        TargetBasicBlockEmitBefore(&rv->base, block, label, base);
        base->opcode = (TargetOpcode)RV32_OP(auipc);
        base->flags |= RV32_PCREL_HI_RELOC;
        TargetReplaceOperand(inst, 1, label);
        inst->flags |= RV32_PCREL_LO_RELOC;
      } else if (base->opcode == (TargetOpcode)RV32_OP(addi)) {
        // Load from an address calculated using an addi instruction.  See if we
        // can combine them.
        int offset = RV32IntValue(inst->operand[1]);
        int immed = RV32IntValue(base->operand[1]);
        if (RV32IsPossibleImmediate(offset + immed)) {
    ...
    } else if (RV32IsStore(inst)) {
      TargetInstruction* base = inst->operand[1];
      if (base->opcode == (TargetOpcode)RV32_OP(la) && !compiler->pic &&
          RV32IntValue(inst->operand[2]) == 0 && base->users.length == 1) {
        ...
        // can combine them.
        int offset = RV32IntValue(inst->operand[2]);
        int immed = RV32IntValue(base->operand[1]);
        if (RV32IsPossibleImmediate(offset + immed)) {
...
static void CombineLoadOrStores(RV32Generator* rv) {
  struct OptimizerData data = {rv};
...
static void PropagateZeroesInBlock(TargetBasicBlock* block, void* data) {
  ...
  for (TargetInstruction* inst = TargetBasicBlockRBegin(block);
    ...
    for (int i = 0; i < TARGET_MAX_OPERANDS; i++) {
      ...
      if (operand != NULL) {
        if (((int)operand->opcode == (int)RV32_OP(mv)) && operand->users.length == 1) {
          TargetInstruction* mv = operand;
          if (mv->operand[0]->opcode == (TargetOpcode)RV32_OP(x0)) {
...
static void PropagateZeroes(RV32Generator* rv) {
  struct OptimizerData data = {rv};
...
static bool CanPoolConstant(TargetInstruction* inst, int op) {
  RV32Opcode opcode = (RV32Opcode)inst->opcode;
  if (RV32IsLoad(opcode)) {
    // Loads can't use a pooled constant.
    return false;
  }
  if (RV32IsStore(opcode)) {
...
static void PoolConstantsInBlock(TargetBasicBlock* block, void* data) {
  ...
  for (TargetInstruction* inst = TargetBasicBlockBegin(block);
    ...
    if (inst->opcode == (TargetOpcode)RV32_OP(li) && inst->dest == NULL) {
      // Possible instruction to pool.
      int64_t value = RV32IntValue(inst->operand[0]);
...
static COMPILER_UNUSED void PoolConstants(RV32Generator* rv) {
...
static void EliminateMovesInBlock(TargetBasicBlock* block, void* data) {
  RV32Generator* rv = data;
  TargetInstruction* next;
  for (TargetInstruction* inst = TargetBasicBlockBegin(block);
      !TargetBasicBlockIsEmpty(block) && inst != TargetBasicBlockEnd(block);
       inst = next) {
    next = TargetNext(inst);
    if (inst->opcode == (TargetOpcode)RV32_OP(mv)) {
      ...
      for (TargetInstruction* prev = TargetPrev(inst);
        ...
        if (RV32IsVarRegister(inst_dest) &&
            RV32IsExpression(prev) && prev->users.length == 1) {
          ...
          TrapRemoveInstruction(inst);
          TargetBasicBlockRemoveInstruction(&rv->base, block, inst);
        } else if (prev->opcode == (TargetOpcode)RV32_OP(mv)) {
...
static void EliminateMoves(RV32Generator* rv) {
  TargetTraverseDominatorTree(&rv->base, EliminateMovesInBlock, kTraversePostOrder, rv);
}

void RV32Optimize(RV32Generator* rv) {  
  // Eliminate moves if we can.
  EliminateMoves(rv);

  // RV32PrintBasicBlocks(rv, stdout);
