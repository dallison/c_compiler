...
// instructions to the given file in assembly language.  The RV32Assember
// reads this file and generates the binary.

#include "risc_v32_emitter.h"
#include <assert.h>
#include <limits.h>
#include <stdlib.h>
#include <string.h>
...
static int VarargsSaveBytes(const RV32Emitter* emitter) {
  if (!emitter->rv->base.varargs) {
    return 0;
  }
  int named = emitter->rv->num_int_arg_regs;
  if (named >= RV32_NUM_INT_ARGS) {
    return 0;
  }
  int remaining = RV32_NUM_INT_ARGS - named;
  int pad = (named & 1) ? 4 : 0;
...
static bool HasExceptionStructReturn(RV32Emitter* emitter) {
  return emitter->rv->struct_return_reg >= 0 &&
         emitter->rv->exception_ranges.length > 0;
}

static int StackFrameSize(RV32Emitter* emitter) {
...
static bool EmptyStackFrame(RV32Emitter* emitter) {
...
static void DecrementStackPointer(RV32Emitter* emitter, int stack_frame_size,
                                  FILE* fp) {
...
static COMPILER_UNUSED void IncrementStackPointer(RV32Emitter* emitter, int stack_frame_size,
                                  FILE* fp) {
  if (stack_frame_size > 0x7ff) {
...
static COMPILER_UNUSED void LoadRegisterFromFrame(RV32Emitter* emitter, int reg,
                                  int offset, bool is_fp,
                                  const char* symbol_name,
                                  FILE* fp) {
  char buf[256];
  const char* instruction = is_fp ? "fld" : "lw";
  RV32RegisterType reg_type = is_fp ? kRV32RegTypeFloat : kRV32RegTypeInt;
  if (offset < 0x7ff) {
    fprintf(fp, "\tf%s %s, -%d(s0)",
            instruction,
            RV32RegisterNameFromNum(reg,
...
static COMPILER_UNUSED void GenerateOffsetFromFrame(RV32Emitter* emitter, int reg,
                                    int offset,
                                    const char* symbol_name,
                                    FILE* fp) {
  char buf[256];
  if (offset < 0x7ff) {
    fprintf(fp, "\taddi %s, s0, -%d",
            RV32RegisterNameFromNum(reg,
                                  kRV32RegTypeInt, buf, sizeof(buf)),
            offset);
  } else {
    fprintf(fp, "\tli t0, %d\n", offset);
...
static void SaveRegisters(RV32Emitter* emitter, FILE* fp) {
  // Entry sequence:
  ...
  bool is_leaf = emitter->rv->base.num_calls == 0 && OptLevel1() &&
          !emitter->rv->not_leaf;
  bool varargs = emitter->rv->base.varargs;
  int space_above_frame_pointer = VarargsSaveBytes(emitter);
  ...
  int saved_reg_offset = stack_frame_size - RV32_STACK_FRAME_HEADER_SIZE - 8 -
                          emitter->rv->base.stack_frame_size -
  ...
  for (size_t i = 0; i < emitter->rv->saved_regs.length; i++) {
    ...
    fprintf(fp, "\t%s %s, %d(%s)\n",
            store,
            RV32RegisterNameFromNum(saved_reg->reg_num,
                                  saved_reg->is_fp ? kRV32RegTypeFloat
                                                   : kRV32RegTypeInt,
                                  buf1, sizeof(buf1)),
            offset,
            RV32RegisterNameFromNum(saved_reg->base_reg_num, kRV32RegTypeInt,
                                  buf2, sizeof(buf2)));
  }
  ...
  while (!BitSetIteratorDone(&it)) {
    ...
    fprintf(fp, "\tsw %s, %d(sp)\n",
            RV32RegisterNameFromNum(reg, kRV32RegTypeInt, buf1, sizeof(buf1)),
            offset);
  ...
  while (!BitSetIteratorDone(&it)) {
    ...
    fprintf(fp, "\tfsd %s, %d(sp)\n",
            RV32RegisterNameFromNum(reg, kRV32RegTypeFloat, buf1, sizeof(buf1)),
            offset);
    BitSetIteratorNext(&it);
  }

  if (HasExceptionStructReturn(emitter)) {
    fprintf(fp, "\tsw a0, %d(sp)\t// hidden result pointer\n",
            saved_reg_offset);
  }
...
static void RestoreRegisters(RV32Emitter* emitter, FILE* fp) {
  // Exit sequence:
  ...
  if (EmptyStackFrame(emitter)) {
    ...
    if (!is_leaf) {
      fprintf(fp, "\tlw ra, -8(s0)\n");
      fprintf(fp, "\tlw s0, -16(s0)\n");
    } else {
      fprintf(fp, "\tlw s0, -8(s0)\n");
    }
    // IncrementStackPointer(emitter, stack_frame_size, fp);
  }
}

static void RestoreExceptionLandingState(RV32Emitter* emitter, FILE* fp) {
  fprintf(fp, "\tmv sp, s0\n");
  ...
  if (emitter->rv->struct_return_reg >= 0) {
    bool is_leaf = emitter->rv->base.num_calls == 0 && OptLevel1() &&
                   !emitter->rv->not_leaf;
    struct_return_phys =
        (is_leaf ? RV32_FIRST_LEAF_INT_REG_VAR : RV32_FIRST_INT_REG_VAR) +
  ...
  if (struct_return_phys >= 0 && HasExceptionStructReturn(emitter)) {
    ...
    fprintf(fp, "\tlw %s, %d(sp)\t// hidden result pointer\n",
            RV32RegisterNameFromNum(struct_return_phys, kRV32RegTypeInt, buf,
                                  sizeof(buf)),
            offset);
  }
}
...
static const char* MoveMnemonic(RV32Opcode opcode, RV32Register* dest,
                                RV32Register* src) {
  if (dest->type == kRV32RegTypeFloat || src->type == kRV32RegTypeFloat) {
    if (opcode == RV32_OP(fmv_d)) {
      return "fmv.d";
    }
    return "fmv.s";
  }
...
static void PrintRmov(RV32Emitter* emitter, TargetInstruction* inst, FILE* fp) {
  ...
  const char* mnemonic =
      MoveMnemonic((RV32Opcode)inst->opcode, (RV32Register*)inst->operand[0]->reg,
                   (RV32Register*)inst->operand[1]->reg);
  char buf1[8], buf2[8];
  fprintf(
      fp, "\t%-12s%s, %s\n", mnemonic,
      RV32RegisterName((RV32Register*)inst->operand[0]->reg, buf1, sizeof(buf1)),
      RV32RegisterName((RV32Register*)inst->operand[1]->reg, buf2, sizeof(buf2)));
}
...
static void PrintDestMove(TargetInstruction* inst, FILE* fp) {
  ...
  const char* mnemonic = MoveMnemonic((RV32Opcode)inst->opcode,
                                      (RV32Register*)dest_reg,
                                      (RV32Register*)src->reg);
  char buf1[8], buf2[8];
  fprintf(fp, "\t%-12s%s, %s\n", mnemonic,
          RV32RegisterName((RV32Register*)dest_reg, buf1, sizeof(buf1)),
          RV32RegisterName((RV32Register*)src->reg, buf2, sizeof(buf2)));
}
...
static const char* GetRegisterName(TargetInstruction* inst, char* buf, size_t size) {
  ...
  if (inst->reg != NULL) {
    return RV32RegisterName((RV32Register*)inst->reg, buf, size);
  }
  if (inst->dest != NULL && inst->dest->reg != NULL) {
    return RV32RegisterName((RV32Register*)inst->dest->reg, buf, size);
  }
  switch ((RV32Opcode)inst->opcode) {
    case RV32_OP(x0):
      return RV32RegisterNameFromNum(RV32_INT_ZERO_REG, kRV32RegTypeInt, buf, size);
    case RV32_OP(t0):
      return RV32RegisterNameFromNum(RV32_INT_TEMP_START_1, kRV32RegTypeInt, buf,
                                   size);
    case RV32_OP(t1):
      return RV32RegisterNameFromNum(RV32_INT_TEMP_START_1 + 1, kRV32RegTypeInt, buf,
                                   size);
    case RV32_OP(t2):
      return RV32RegisterNameFromNum(RV32_INT_TEMP_START_1 + 2, kRV32RegTypeInt, buf,
                                   size);
    case RV32_OP(fp):
      return RV32RegisterNameFromNum(RV32_FP_REG, kRV32RegTypeInt, buf, size);
    case RV32_OP(sp):
      return RV32RegisterNameFromNum(RV32_SP_REG, kRV32RegTypeInt, buf, size);
    case RV32_OP(a0):
    case RV32_OP(a1):
    case RV32_OP(a2):
    case RV32_OP(a3):
    ...
    case RV32_OP(a6):
    case RV32_OP(a7):
      return RV32RegisterNameFromNum(
          (int)inst->opcode - RV32_OP(a0) + RV32_INT_ARG_START, kRV32RegTypeInt, buf,
          size);
    case RV32_OP(resulti):
    case RV32_OP(call):
    case RV32_OP(rcall):
      return RV32RegisterNameFromNum(RV32_INT_RETURN_REG, kRV32RegTypeInt, buf, size);
    case RV32_OP(resultf):
    case RV32_OP(resultd):
    case RV32_OP(callf):
    case RV32_OP(rcallf):
      return RV32RegisterNameFromNum(RV32_FLOAT_RETURN_REG, kRV32RegTypeFloat, buf,
                                   size);
    default:
      return "";
  }
...
static AsmOperand* GetAsmOperand(RV32AsmInstruction* inst, int index) {
...
static int AtomicSizeLog2(TargetInstruction* inst) {
  return (inst->flags & RV32_ATOMIC_SIZE_MASK) >> RV32_ATOMIC_SIZE_SHIFT;
}

static int AtomicOrder(TargetInstruction* inst) {
  return (inst->flags & RV32_ATOMIC_ORDER_MASK) >> RV32_ATOMIC_ORDER_SHIFT;
}

static int AtomicFailureOrder(TargetInstruction* inst) {
  return (inst->flags & RV32_ATOMIC_FAILURE_ORDER_MASK) >>
         RV32_ATOMIC_FAILURE_ORDER_SHIFT;
}
...
static void PrintAtomicInstruction(TargetInstruction* inst,
  ...
  switch ((RV32Opcode)inst->opcode) {
    ...
    case RV32_OP(atomic_sub_fetch): {
      bool add = inst->opcode == (TargetOpcode)RV32_OP(atomic_fetch_add) ||
                 inst->opcode == (TargetOpcode)RV32_OP(atomic_add_fetch);
      bool return_new =
          inst->opcode == (TargetOpcode)RV32_OP(atomic_add_fetch) ||
          inst->opcode == (TargetOpcode)RV32_OP(atomic_sub_fetch);
      const char* addr =
      ...
      fprintf(fp, "\tamoadd.w%s %s, t0, (%s)\n",
              AtomicSuffix(acquire, release), result, addr);
      if (return_new) {
    ...
    case RV32_OP(atomic_compare_exchange_n): {
      bool expected_is_pointer =
          inst->opcode == (TargetOpcode)RV32_OP(atomic_compare_exchange_n);
      bool returns_bool =
          inst->opcode != (TargetOpcode)RV32_OP(atomic_compare_exchange_val);
      bool lr_acquire =
      ...
      if (size < 2) {
        ...
        fprintf(fp, "\tsc.w%s t6, t5, (t0)\n",
                AtomicSuffix(order == 5, release));
        if ((inst->flags & RV32_ATOMIC_WEAK) != 0) {
...
static int FindAsmOperandByName(RV32AsmInstruction* inst, const char* name,
                                size_t len) {
...
static int FindAsmLabelByName(RV32AsmInstruction* inst, const char* name,
                              size_t len) {
...
static void PrintAsmOperand(FILE* fp, RV32AsmInstruction* inst, int index) {
  if (index < 0 || index >= inst->num_operands) {
  ...
  char buf[32];
  fprintf(fp, "%s", RV32RegisterNameFromNum(inst->reg_nums[index],
                                          inst->is_fp[index] ? kRV32RegTypeFloat
                                                             : kRV32RegTypeInt,
                                          buf, sizeof(buf)));
}

static void PrintAsmLabel(FILE* fp, RV32AsmInstruction* inst, int index) {
...
static void PrintExtendedAsm(FILE* fp, RV32AsmInstruction* inst,
                             const char* text) {
...
static void PrintInstruction(RV32Emitter* emitter, TargetInstruction* inst,
                             const char* func_name, FILE* fp) {
  if (inst->block == NULL) {
    return;
  }
  ...
  if (((int)inst->opcode == (int)RV32_OP(label))) {
    if ((inst->flags & RV32_EXPORTED_LABEL) != 0) {
  ...
  if (((int)inst->opcode == (int)RV32_OP(named_label))) {
    TargetNamedLabel* label = (TargetNamedLabel*)inst;
    fprintf(fp, "%s:\n", label->name);
    return;
  }
  
  if (((int)inst->opcode == (int)RV32_OP(ivarreg)) || ((int)inst->opcode == (int)RV32_OP(fvarreg))) {
    return;
  }
  if (!IsPrintable(inst)) {
    return;
...
//      return;
    ...
    case RV32_OP(call):
    case RV32_OP(callf): {
      assert(((int)inst->operand[0]->opcode == (int)RV32_OP(symbol)));
    ...
    case RV32_OP(tlsgd): {
      assert(inst->operand[0] != NULL &&
             inst->operand[0]->opcode == (TargetOpcode)RV32_OP(symbol));
      TargetSymbol* sym = (TargetSymbol*)inst->operand[0];
      fprintf(fp, "\t%-12s%s, %s\n",
              (RV32Opcode)inst->opcode == RV32_OP(tlsgd) ? "tlsgd" : "tprel",
    ...
    case RV32_OP(asm): {
      ...
      fprintf(fp, "\t");
      if ((inst->flags & RV32_INST_EXTENDED_ASM) != 0) {
        PrintExtendedAsm(fp, (RV32AsmInstruction*)inst, lit->value.value);
      } else {
    ...
    case RV32_OP(spill): {
      RV32Register* reg = (RV32Register*)inst->reg;
      int offset = (int)TargetIntValue(inst->operand[1]) + emitter->first_spill_offset;
      const int spill_addr = RV32_SPILL_ADDR;
      if (!RV32IsPossibleImmediate(offset)) {
        fprintf(fp, "\t%-12s%s, %d\n",
                "li",
                RV32RegisterNameFromNum(spill_addr, kRV32RegTypeInt, buf1, sizeof(buf1)),
                offset);
        fprintf(fp, "\t%-12s%s, s0, %s\n",
                "sub",
                RV32RegisterNameFromNum(spill_addr, kRV32RegTypeInt, buf1, sizeof(buf1)),
                RV32RegisterNameFromNum(spill_addr, kRV32RegTypeInt, buf2, sizeof(buf2)));
        fprintf(fp, "\t%-12s%s, 0(%s)\t// Spilled @%d\n",
                 reg->type == kRV32RegTypeInt ? "sw" : "fsd",
                 RV32RegisterName(reg, buf1, sizeof(buf1)),
                 RV32RegisterNameFromNum(spill_addr, kRV32RegTypeInt, buf2, sizeof(buf2)),
                 inst->operand[0]->id);
      } else {
    ...
    case RV32_OP(reload): {
      ...
      if (!RV32IsPossibleImmediate(offset)) {
        ...
        fprintf(fp, "\t%-12s%s, 0(%s)\t// Reloaded spilled @%d\n",
                 reg->type == kRV32RegTypeInt ? "lw" : "fld",
                 RV32RegisterName(reg, buf1, sizeof(buf1)),
                 RV32RegisterNameFromNum(spill_addr, kRV32RegTypeInt, buf2, sizeof(buf2)),
                 spill->operand[0]->id);
      } else {
  ...
  // Print opcode.
  fprintf(fp, "\t%-12s", RV32OpcodeName(inst->opcode));
  ...
  switch ((RV32Opcode)inst->opcode) {
    ...
    case RV32_OP(fld):
      ...
      if (TargetIsConst(inst->operand[1])) {
        ...
        assert(RV32IsPossibleImmediate(offset));
        fprintf(fp, "%s, %d(%s)\n",
                GetRegisterName(inst, buf1, sizeof(buf1)),
                offset,
                GetRegisterName(inst->operand[0], buf2,
      ...
      } else if (((int)inst->operand[1]->opcode == (int)RV32_OP(symbol))) {
        assert((inst->flags & RV32_LO_RELOC) != 0);
        fprintf(fp, "%s, %%lo(%s)(%s)\n",
                GetRegisterName(inst, buf1, sizeof(buf1)),
                SymbolName((TargetSymbol*)inst->operand[1], buf3, sizeof(buf3)),
                GetRegisterName(inst->operand[0], buf2,
                               sizeof(buf2)));
      } else if (((int)inst->operand[1]->opcode == (int)RV32_OP(label))) {
        assert((inst->flags & RV32_PCREL_LO_RELOC) != 0);
        fprintf(fp, "%s, %%pcrel_lo(.%s_label_%d)(%s)\n",
                GetRegisterName(inst, buf1, sizeof(buf1)),
                func_name, inst->operand[1]->id,
                GetRegisterName(inst->operand[0], buf2,
                               sizeof(buf2)));
      } else if (((int)inst->operand[1]->opcode == (int)RV32_OP(x0))) {
        fprintf(fp, "%s, 0(%s)\n",
                GetRegisterName(inst, buf1, sizeof(buf1)),
                GetRegisterName(inst->operand[0], buf2,
                               sizeof(buf2)));
      ...
      if (TargetIsConst(inst->operand[2])) {
        ...
        fprintf(fp, "%s, %d(%s)\n",
                ...
                GetRegisterName(inst->operand[1], buf2,
                               sizeof(buf2)));
      } else if (((int)inst->operand[2]->opcode == (int)RV32_OP(symbol))) {
        assert((inst->flags & RV32_LO_RELOC) != 0);
        fprintf(fp, "%s, %%lo(%s)(%s)\n",
                GetRegisterName(inst->operand[0], buf1,
                               sizeof(buf1)),
                ...
                GetRegisterName(inst->operand[1], buf2,
                               sizeof(buf2)));
      } else if (((int)inst->operand[2]->opcode == (int)RV32_OP(label))) {
        assert((inst->flags & RV32_PCREL_LO_RELOC) != 0);
        fprintf(fp, "%s, %%pcrel_lo(.%s_label_%d)(%s)\n",
                GetRegisterName(inst->operand[0], buf1,
                               sizeof(buf1)),
                ...
                GetRegisterName(inst->operand[1], buf2,
                               sizeof(buf2)));
      } else if (((int)inst->operand[2]->opcode == (int)RV32_OP(x0))) {
        fprintf(fp, "%s, 0(%s)\n",
                GetRegisterName(inst->operand[0], buf1,
                               sizeof(buf1)),
                GetRegisterName(inst->operand[1], buf2,
    ...
    case RV32_OP(j): {
      ...
      if (((int)dest->opcode == (int)RV32_OP(label))) {
        fprintf(fp, ".%s_label_%d\n", func_name, inst->operand[0]->id);
      } else if (((int)dest->opcode == (int)RV32_OP(symbol))) {
        fprintf(fp, "%s\n", SymbolName((TargetSymbol*)dest, buf3, sizeof(buf3)));
      } else if (((int)dest->opcode == (int)RV32_OP(named_label))) {
    ...
    default: {
      ...
      for (int i = 0; i < 2; i++) {
        if (inst->operand[i] != NULL) {
          ...
          } else if (((int)inst->operand[i]->opcode == (int)RV32_OP(symbol))) {
            if ((inst->flags & RV32_HI_RELOC) != 0) {
...
RV32Emitter* NewRV32Emitter(RV32Generator* rv) {
  RV32Emitter* emitter = malloc(sizeof(RV32Emitter));
  RV32EmitterInit(emitter, rv);
  return emitter;
}
...
static void RV32FillLSDAInfo(RV32Emitter* emitter, DaveEHLSDARange* lsda_ranges,
                           size_t* count) {
  *count = 0;
  for (size_t i = 0; i < emitter->rv->exception_ranges.length && *count < 64;
       i++) {
    RV32ExceptionRange* range = emitter->rv->exception_ranges.value.p[i];
...
static size_t RV32SavedCFIRegisters(RV32Emitter* emitter,
...
static void RV32PrintEHMetadata(RV32Emitter* emitter, FILE* fp,
                              const char* func_name) {
  if (emitter->rv->base.varargs) {
    return;
  }
  DaveEHLSDARange lsda_ranges[64];
  DaveEHFrameSavedReg saved_regs[RV32_NUM_INT_REGS];
  size_t lsda_count = 0;
  size_t saved_reg_count =
      RV32SavedCFIRegisters(emitter, saved_regs,
                          sizeof(saved_regs) / sizeof(saved_regs[0]));
  RV32FillLSDAInfo(emitter, lsda_ranges, &lsda_count);
...
void RV32PrintFunction(RV32Emitter* emitter, FILE* fp) {
  ...
  fprintf(fp, "\t.size %s, .func_end_%s-%s\n\n", func_name, func_name,
          func_name);
  RV32PrintEHMetadata(emitter, fp, func_name);
}
