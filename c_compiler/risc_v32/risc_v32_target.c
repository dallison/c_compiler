...
static void* GenerateCode(Generator* gen) {
  RV32Generator* rv = NewRV32Generator(gen);
  RV32Lower(rv, gen);
  return rv;
}
...
static void EmitFunctionAssembly(void* code, FILE* asm_file) {
  RV32Emitter emitter;
  RV32EmitterInit(&emitter, (RV32Generator*)code);
  RV32PrintFunction(&emitter, asm_file);
...
static bool Assemble(String* asm_filename, String* object_filename) {
  ...
  RV32AssemblerInit(&assembler, asm_filename, object_filename);
  AssemblerRun(&assembler.base, AssembleRV32Instruction);
...
static void Cleanup(void* code) { RV32GeneratorDelete(code); }

static void HandleOptions(Vector* options) {
  
}

// Create a new RV32 target.  The functions are called by the
// compiler.
CompilerTarget* NewRV32Target() {
  CompilerTarget* target = malloc(sizeof(CompilerTarget));
  StringInit(&target->name, "riscv32");
  ...
  target->emit_cxx_thunks = RV32PrintCXXAdjustorThunks;
