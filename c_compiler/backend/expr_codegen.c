...
static IRNode* RemoveUnnecesaryShortening(Generator* gen, IRNode* value,
                                           IROpcode store);
static IRNode* NormalizeBitIntValue(Generator* gen, IRNode* value,
                                    TypeRecord* type);

static IRNode* AtomicSeqCstOrder(Generator* gen) {
  TypeRecord* int_type = NewTypeRecordWithSize(kTypeInt, kQualPlain);
  return GeneratorGetIntConstant(gen, int_type, 5);
}

static TypeRecord* AtomicValueType(TypeRecord* type) {
  TypeRecord* value_type = TypeRecordCopy(type);
  if (value_type != NULL) {
...
static IRNode* AtomicObjectAddress(Generator* gen, IRNode* address,
                                   TypeRecord* object_type) {
  if (IRIsVariable(address)) {
    address = GeneratorEmit(gen, NewIR1(IR_OP(addressof), address));
  }
  return IRSetType(address, NewPointerTo(kQualPlain, object_type));
}
...
static IRNode* EmitObjectStore(Generator* gen, ASTNode* node, IRNode* address,
                               IRNode* value) {
  IROpcode ordinary_store = GetStoreOpcode(node);
  value = RemoveUnnecesaryShortening(gen, value, ordinary_store);
  if (!TypeIsAtomic(node->type)) {
    return GeneratorEmit(gen, NewIR2(ordinary_store, address, value));
  }
  address = AtomicObjectAddress(gen, address, node->type);
  IRNode* store = GeneratorEmit(
      gen, NewIR3(IR_OP(atomic_store), address, value, AtomicSeqCstOrder(gen)));
...
static IRNode* GenerateBinaryExpression(Generator* gen, BinaryASTNode* node) {
  if (TypeIsVector(node->left->type) ||
    ...
    IROpcode opcode = IR_OP(nop);
    switch (node->base.op) {
      case AST_OP(plus): opcode = IR_OP(vadd); break;
      case AST_OP(minus): opcode = IR_OP(vsub); break;
      case AST_OP(mult): opcode = IR_OP(vmul); break;
      case AST_OP(div): opcode = IR_OP(vdiv); break;
      case AST_OP(mod): opcode = IR_OP(vmod); break;
      case AST_OP(lshift): opcode = IR_OP(vlsl); break;
      case AST_OP(rshiftl): opcode = IR_OP(vlsr); break;
      case AST_OP(rshifta): opcode = IR_OP(vasr); break;
      case AST_OP(and): opcode = IR_OP(vand); break;
      case AST_OP(bitor): opcode = IR_OP(vor); break;
      case AST_OP(exor): opcode = IR_OP(vxor); break;
      case AST_OP(equal): opcode = IR_OP(vcmpeq); break;
      case AST_OP(noteq): opcode = IR_OP(vcmpne); break;
      case AST_OP(less): opcode = IR_OP(vcmplt); break;
      case AST_OP(lesseq): opcode = IR_OP(vcmple); break;
      case AST_OP(greater): opcode = IR_OP(vcmpgt); break;
      case AST_OP(greatereq): opcode = IR_OP(vcmpge); break;
      default:
        assert(false && "unsupported vector binary operation");
    }
    if (TypeIsUnsigned(TypeVectorElement(node->left->type))) {
      switch (opcode) {
        case IR_OP(vcmplt): opcode = IR_OP(vcmpltu); break;
        case IR_OP(vcmple): opcode = IR_OP(vcmpleu); break;
        case IR_OP(vcmpgt): opcode = IR_OP(vcmpgtu); break;
        case IR_OP(vcmpge): opcode = IR_OP(vcmpgeu); break;
        default: break;
      }
    }
    ...
    temporary->flags.address_taken = true;
    IRNode* destination = GeneratorGetVariable(gen, temporary);
    IRNode* operation =
        IRSetType(GeneratorEmit(gen, NewIR3(opcode, destination, left, right)),
                  node->base.type);
...
static IRNode* GenerateUnaryExpression(Generator* gen, UnaryASTNode* node) {
  ...
  if (TypeIsVector(node->sub->type)) {
    ...
    IROpcode opcode = node->base.op == AST_OP(uminus)
                          ? IR_OP(vneg)
                          : IR_OP(vonescomp);
    Symbol* temporary = SyntaxNewTemporary(gen->syntax, node->base.type);
    temporary->flags.address_taken = true;
    IRNode* destination = GeneratorGetVariable(gen, temporary);
    IRSetType(GeneratorEmit(gen, NewIR2(opcode, destination, sub)),
              node->base.type);
    return destination;
  }
...
static IRNode* GenerateVariableReference(Generator* gen,
  ...
  if ((node->base.flags & kASTNeedAddress) != 0 ||
    ...
    result = EmitObjectLoad(gen, &node->base, var_ref);
    if (!TypeIsAtomic(node->base.type)) {
      IRSetVarUse(result, node->symbol);
    }
  }
...
//      }
  ...
  // If we are operating on a bitfield, we need to set the value back into the
   ...
   } else if (TypeIsBitInt(node->base.type)) {
     new_value = NormalizeBitIntValue(gen, new_value, node->base.type);
   }
...
static IRNode* GenerateAtomicIncDec(Generator* gen, UnaryASTNode* node,
                                    bool is_post, bool is_inc) {
  IRNode* address = GenerateExpression(gen, node->sub);
  address = AtomicObjectAddress(gen, address, node->sub->type);
  int amount = TypeIsPointer(node->sub->type) && node->sub->type->next != NULL
                   ? node->sub->type->next->size
                   : 1;
  TypeRecord* value_type = AtomicValueType(node->sub->type);
  IRNode* increment = GeneratorGetIntConstant(gen, value_type, amount);
  IROpcode opcode =
      is_inc ? (is_post ? IR_OP(atomic_fetch_add) : IR_OP(atomic_add_fetch))
             : (is_post ? IR_OP(atomic_fetch_sub) : IR_OP(atomic_sub_fetch));
  IRNode* result = GeneratorEmit(
      gen, NewIR3(opcode, address, increment, AtomicSeqCstOrder(gen)));
  IRSetType(result, AtomicValueType(node->sub->type));
  return result;
}
...
static void GenerateBracedInitializer(Generator* gen, ASTNode* node,
  ...
  for (size_t i = 0; i < init->initializers->length; i++) {
    ...
    if (subinit->op == AST_OP(designated_init)) {
      ...
      assert(aggregate_type != NULL &&
             (TypeIsArray(aggregate_type) || TypeIsVector(aggregate_type)));
    ...
    if (TypeIsStructOrUnion(designated_init->base.type) ||
        TypeIsVector(designated_init->base.type) ||
    ...
    if (TypeIsStructOrUnion(designated_init->base.type) ||
      ...
      if (destination_was_zeroed && init->base.type != NULL &&
          (TypeIsArray(init->base.type) || TypeIsVector(init->base.type) ||
           TypeIsStructOrUnion(init->base.type) ||
...
static IRNode* GenerateAtomicCompoundAssignment(Generator* gen,
  ...
  IRNode* result = GeneratorEmit(
      gen, NewIR3(add ? IR_OP(atomic_add_fetch) : IR_OP(atomic_sub_fetch),
                  address, value, AtomicSeqCstOrder(gen)));
  IRSetType(result, AtomicValueType(node->left->type));
  return AssignmentResultIsOuterAssignmentLHS((ASTNode*)node) ? address
                                                               : result;
}
...
static IRNode* GenerateCompoundAssignment(Generator* gen,
                                           BinaryASTNode* node) {
  if (TypeIsAtomic(node->left->type) &&
      (node->base.op == AST_OP(pluseq) ||
       node->base.op == AST_OP(minuseq))) {
    return GenerateAtomicCompoundAssignment(gen, node);
  }
  bool result_address_needed =
      AssignmentResultIsOuterAssignmentLHS((ASTNode*)node);
...
static IRNode* GenerateIndexExpression(Generator* gen, BinaryASTNode* node) {
  ...
  IRNode* result = EmitObjectLoad(gen, (ASTNode*)node, addr);
  if (!TypeIsAtomic(node->base.type)) {
...
static IRNode* GenerateFunctionCall(Generator* gen, VectorASTNode* node) {
  ...
  for (ssize_t i = node->children->length - 1; i >= 0; i--) {
    ...
    if (cxx_constructor_call && i == 0 &&
      ...
      if (reference_formal &&
          ...
          !TypeIsStructOrUnion(arg->type) && !TypeIsVector(arg->type)) {
    ...
    bool native_vector_value =
        !reference_formal &&
        TypeUsesNativeVectorABI(formal_type != NULL ? formal_type : arg->type);
    if (native_vector_value) {
      arg_value =
          IRSetType(GeneratorEmit(gen, NewIR1(IR_OP(vectorarg), arg_value)),
                    formal_type != NULL ? formal_type : arg->type);
      CheckForVarUse(arg_value, arg);
  ...
  if (TypeUsesNativeVectorABI(node->base.type)) {
    IRNode* destination;
    if (gen->current_struct_address != NULL) {
      destination = FreshCallAddress(gen, gen->current_struct_address);
    } else {
      Symbol* temporary = SyntaxNewTemporary(gen->syntax, node->base.type);
      temporary->flags.address_taken = true;
      destination = GeneratorGetVariable(gen, temporary);
    }
    IRSetType(GeneratorEmit(gen, NewIR2(IR_OP(capturev), destination, call)),
              node->base.type);
    return destination;
  }
...
static IRNode* GenerateContentsOf(Generator* gen, UnaryASTNode* node) {
  ...
  return EmitObjectLoad(gen, (ASTNode*)node, addr);
}
...
static IRNode* GenerateMemberReference(Generator* gen, BinaryASTNode* node) {
  ...
  if (member->member->is_static) {
    ...
    IRNode* load = EmitObjectLoad(gen, (ASTNode*)node, var_ref);
    if (!TypeIsAtomic(node->base.type)) {
  ...
  if (StructMemberIsBitField(member->member)) {
    ...
    IRNode* load = EmitObjectLoad(gen, (ASTNode*)node, addr);
    if (!TypeIsAtomic(node->base.type)) {
      CheckForVarUse(load, (ASTNode*)node);
    }
    return load;
...
static IRNode* GenerateBuiltinAtomic(Generator* gen, VectorASTNode* node,
  ...
  bool needs_default_order =
      (opcode == IR_OP(atomic_fence) && node->children->length == 0) ||
      ((opcode == IR_OP(atomic_fetch_add) ||
        opcode == IR_OP(atomic_fetch_sub) ||
        opcode == IR_OP(atomic_add_fetch) ||
        opcode == IR_OP(atomic_sub_fetch)) &&
...
static IRNode* GenerateSoftwarePopcount(Generator* gen, IRNode* value,
                                        ...
                                        TypeRecord* result_type, int width) {
...
static IRNode* GenerateBuiltinBitOperation(Generator* gen,
  ...
  int width = TypeIsBitInt(value_type) ? value_type->bit_width
                                       : value_type->size * 8;
  ...
  if (op != AST_OP(builtin_rotl) && op != AST_OP(builtin_rotr) &&
    ...
    if (have_width && explicit_width > 0 && explicit_width <= 64) {
  ...
  if (width < value_type->size * 8) {
    value = EmitBitIR2(
        ...
        BitConstant(gen, value_type, (UINT64_C(1) << width) - 1), value_type);
  }
...
static IRNode* GenerateZeroExtend(Generator* gen, ASTNode* node, IRNode* input) {
  int diff = node->type->size - input->type->size;  // Difference in bytes.
   if (diff == 0) {
     return IRSetType(input, node->type);
   }
...
static IRNode* NormalizeBitIntValue(Generator* gen, IRNode* value,
                                    TypeRecord* type) {
  if (value == NULL || !TypeIsBitInt(type) || type->bit_width <= 0) {
    return value;
  }
  int storage_bits = type->size * 8;
  if (type->bit_width >= storage_bits) {
    return IRSetType(value, type);
  }
  uint64_t mask = (UINT64_C(1) << type->bit_width) - UINT64_C(1);
  IRNode* masked = IRSetType(
      GeneratorEmit(
          gen, NewIR2(IR_OP(andi), value,
                      GeneratorGetIntConstant(gen, type, (int64_t)mask))),
      type);
  if (TypeIsUnsigned(type)) {
    return masked;
  }
  int64_t sign_bit = (int64_t)(UINT64_C(1) << (type->bit_width - 1));
  IRNode* biased = IRSetType(
      GeneratorEmit(
          gen, NewIR2(IR_OP(xori), masked,
                      GeneratorGetIntConstant(gen, type, sign_bit))),
      type);
  return IRSetType(
      GeneratorEmit(
          gen, NewIR2(IR_OP(subi), biased,
                      GeneratorGetIntConstant(gen, type, sign_bit))),
      type);
}
