...
// Standalone Xtensa / ESP32 assembler that generates ELF object files.
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char* argv[]) {
  ...
  StringInit(&target->value.svalue, "esp32");
  ...
  for (size_t i = 0; i < asm_filenames.length; i++) {
    ...
    XTENSAAssembler* assembler =
        NewXTENSAAssembler(asm_filename, &output_filename);
    if (assembler == NULL) {
      fprintf(stderr, "Failed to initialize Xtensa assembler\n");
      exit(1);
    }
    AssemblerRun(&assembler->base, AssembleXTENSAInstruction);

    int num_errors = assembler->base.num_errors;
    XTENSAAssemblerDelete(assembler);
    if (num_errors != 0) {
      exit(1);
    }
